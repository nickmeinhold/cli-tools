...
# belief), and Telegrams Nick a ranked digest ONLY when something crosses the
...
PARALLAX_DIR="$HOME/git/tools/cli-tools/parallax"
LOG="$PARALLAX_DIR/.state/nightly.log"
JSON="$PARALLAX_DIR/.state/last-scan.json"
TG_TO="2122439699"      # Nick's own account id (self-DM). Numeric: `telegram send`
                        # returns RC 0 even on a name-miss, so never use a name here.
THRESHOLD="${PARALLAX_THRESHOLD:-0.5}"   # min top-finding nats to bother Nick
