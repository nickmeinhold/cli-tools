...
export const HEARTBEATS = [
  {
    name: "nightly-backup",
    probe: { file: h(".claude", "backup.log") },
    expectedMaxAgeH: 30, // cron 0 3 * * * (daily) + slack
    blast: 0.9, // losing backups is catastrophic-but-silent
  },
  {
    name: "session-sync",
    probe: { file: h("git", "tools", "claude-sessions", "sync.log") },
    expectedMaxAgeH: 30, // cron 30 3 * * *
    blast: 0.5,
  },
  {
    name: "consolidation",
    probe: { newestUnder: h(".claude", "consolidation") },
    expectedMaxAgeH: 48, // session-triggered, but a 2-day silence is real drift
    blast: 0.6,
  },
  {
    name: "meds-reminder",
    probe: { file: h(".claude", "meds-reminder.log") },
    expectedMaxAgeH: 30, // cron 25 23 * * *  (log may not exist yet -> surfaced as unknown)
    blast: 0.7,
  },
];
...
// A commit authored by one of these (or co-authored) is a PEER Claude instance,
// not entropy and not Nick's hands — the peer-instance-collision doctrine made
