 ...
 *      ~/git/tools/cli-tools/.tokens/telegram/session.txt (mode 0600).
 *
 * Subcommands:
...
Subcommands:
  ...
  send   --to <name|@user|id> --text "..."   Send a message (or --file PATH for the body).
         [--reply-to <msg-id>]                 Quote-reply to a message (id from 'read --json').
...
async function cmdRead(args) {
  ...
  for (const m of msgs) {
    ...
    console.log(`[${ts}] #${m.id} ${who}: ${m.text.replace(/\n/g, "\n            ")}`);
  }
}

async function cmdParticipants(args) {
  const client = await connected();
  try {
    const dialog = await resolveDialog(client, args);
    const entity = dialog.entity || dialog;
    // GramJS getParticipants works on groups/supergroups you're a member of.
    const parts = await client.getParticipants(entity, { limit: args.limit ? parseInt(args.limit, 10) : 1000 });
    const out = parts.map((p) => ({
      name: [p.firstName, p.lastName].filter(Boolean).join(" ") || p.title || "",
      username: p.username || "",
      id: toId(p.id),
      bot: !!p.bot,
    }));
    if (args.json) {
      console.log(JSON.stringify({ group: dialog.name, count: out.length, participants: out }, null, 2));
      return;
    }
    console.error(`# ${dialog.name} — ${out.length} participants\n`);
    for (const p of out) console.log(`  ${p.name}${p.username ? "  @" + p.username : ""}${p.bot ? "  [bot]" : ""}  [${p.id}]`);
  } finally {
    await client.disconnect();
  }
}
...
async function cmdSend(args) {
  ...
  // --reply-to <msg-id> turns this into a quoted reply. The id comes from
  // `telegram read --json` (each message carries its numeric `id`).
  const sendOpts = { message: text };
  if (args["reply-to"]) sendOpts.replyTo = Number(args["reply-to"]);
  const res = await client.sendMessage(entity, sendOpts);
  await client.disconnect();
...
async function main() {
  ...
  switch (sub) {
    ...
    case "participants": return cmdParticipants(args);
