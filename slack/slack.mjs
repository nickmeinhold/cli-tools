 ...
 *     • User token  (xoxp-…) — acts AS Nick. Sees every channel/DM he's in,
 *       and is the ONLY type that can call search.messages. This is what you
 *       want for "read the StartSpace workspace like Nick would".
 ...
 *
 * Credentials (one-time, Nick's hands — user token, the useful case):
 *   1. https://api.slack.com/apps → Create New App → "From scratch" → pick the
 *      StartSpace workspace.
...
AUTH — two paths, tried in order (NO setup needed for path 2):
  ...
  2. The logged-in Slack DESKTOP app (default, zero-setup — like signal/telegram):
     ...
     --workspace startspace. This is the normal path on Nick's machine.
...
EXAMPLES
  slack channels --search startspace
  slack read --channel startspace --from Amir --limit 50
  slack search --query "presentation" --from Amir --limit 20
