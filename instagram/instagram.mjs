 ...
 *   ~/git/tools/cli-tools/.tokens/instagram/storageState.json
 *
 * First-time login:
...
async function cmdSend(args) {
  ...
  // Reply parity with the other chat CLIs is intentionally NOT implemented here:
  // IG is driven through the web UI (no API), and quote-reply requires hovering a
  // specific bubble and clicking "Reply" with no stable message-id handle in the
  // DOM. Faking it would be fragile; fail loudly instead of sending unquoted.
  if (args["reply-to"]) die("--reply-to is not supported on Instagram (web UI has no stable reply handle); send without it.", 2);
