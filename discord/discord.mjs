...
SUBCOMMANDS
  ...
  send        ... [--reply-to <msg-id>]                     (inline reply; id from 'read')
...
try {
  if (cmd === "whoami") {
    ...
    // --reply-to <msg-id> turns this into an inline reply. The id comes from
    // `discord read` (each message carries its `id`).
    const body = { content: args.text };
    if (args["reply-to"]) body.message_reference = { message_id: String(args["reply-to"]) };
