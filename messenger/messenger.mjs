 ...
 *   ~/git/tools/cli-tools/.tokens/messenger/appstate.json
...
AUTH (the Playwright -> appState bridge)
  ws3-fca runs on an appState (FB cookies). Bootstrap interactively (Nick's hands):
    ...
    messenger auth --appstate ~/git/tools/cli-tools/.tokens/playwright/messenger-fb.json
...
async function cmdRead(args) {
  ...
  const msgs = (history || []).map((m) => ({
    id: m.messageID || m.messageId || null,
...
async function cmdSend(args) {
  ...
  // ws3-fca signature: sendMessage(msg, threadID, replyToMessage, isSingleUser).
  // --reply-to <msg-id> quotes a message; id comes from `messenger read --json`.
  const replyTo = args["reply-to"] ? String(args["reply-to"]) : null;
  const info = await api.sendMessage(body, target.id, replyTo, isSingleUser);
  console.log(
    JSON.stringify(
      {
